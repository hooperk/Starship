...
namespace StarshipGenerator
{
    public class Generator
    {
        ...
        public static void Main() 
        {
            ...
            comps.Add(new Supplemental("Main Cargo Hold", HullType.Transport, 2, 4, 1, RuleBook.CoreRulebook, 203, trade: 125));
            comps.Add(new Supplemental("Manufactorum", HullType.AllCruiser, 2, 1, 2, RuleBook.BattlefleetKoronus, 39, null, "+10 to Extended Repair Tests and Acquisition Tests for Repairs. May be able to produce small numbers of personal items", trade: 10, max: 1));
            comps.Add(new Supplemental("Medicae Deck", HullType.All, 2, 1, 1, RuleBook.BattlefleetKoronus, 39, null, "+20 to all Medicae Tests taken within this component. The number of patients may be up to triple the healer's Intelligence Bonus instead of double", max: 1));
            comps.Add(new Supplemental("Melodium", HullType.All, 1, 1, 1, RuleBook.BattlefleetKoronus, 39, null, "+10 to Social Skills Tests", morale: 1, max: 1));
            comps.Add(new Supplemental("Micro Laser Defense Grid", HullType.All, 2, 0, 2, RuleBook.CoreRulebook, 208, null, "External", turrets: 2, comp: ComponentOrigin.Xenotech));
            comps.Add(new Supplemental("Minelayer Bay", HullType.Transport | HullType.AllCruiser, 1, 4, 1, RuleBook.BattlefleetKoronus, 38, null, "May deploy a 4VUx4VUx4VU minefield behind the vessel with a +20 Tech-Use Test as an Extended Action (Even during Stern Chases). Holds enough to deploy 3 mine fields before requiring restocking with an upkeep test at a suitable port. Minefields may not overlap."));
            comps.Add(new Supplemental("Munitorium", ~HullType.AllCruiser, 2, 3, 2, RuleBook.CoreRulebook, 205, null, "If this component is damaged, it explodes. The ship takes 2d5 Hull Integrity damage and a component of the GM's choise is set on fire.", macrodamage: 1, military: 25, max: 1));
            comps.Add(new Supplemental("Munitorium", HullType.AllCruiser, 3, 4, 2, RuleBook.CoreRulebook, 205, null, "If this component is damaged, it explodes. The ship takes 2d5 Hull Integrity damage and a component of the GM's choise is set on fire.", macrodamage: 1, military: 25, max: 1));
            comps.Add(new Supplemental("Murder Servitors", HullType.All, 1, 1, 2, RuleBook.CoreRulebook, 206, null, "+20 to Opposed Command when conducting a Hit and Run Action. When determining the critical hit inflicted by a Hit and Run Action they participated in the raid may select a result between 1 and 6 instead of rolling", max: 1));
            comps.Add(new Supplemental("Null Bay", HullType.All, 1, 2, 1, RuleBook.HostileAcquisition, 73, null, "May hold up to 100 psykers in solitary confinement, imposing -60 to all Focus Power Tests made by them, or tests for other warp based powers such as Navigator Powers. Decrease morale on board the ship by 3 while active", criminal: 50, comp: ComponentOrigin.Archeotech));
            comps.Add(new Supplemental("Observation Dome", HullType.All, 0, 1, 1, RuleBook.CoreRulebook, 206, morale: 1, exploration: 50, max: 1));
            comps.Add(new Supplemental("Pharmacia", HullType.All, 1, 2, 2, RuleBook.HostileAcquisition, 72, null, "May use this component to Synthesise drugs requiring a Chymistry test with a difficulty equal to the compound's availability. May gain 50 achievement points for criminal or trade objectives where providing medical support or recreational pharmaceuticals could assist in negotiations", max: 1));
            comps.Add(new Supplemental("Pilot's Chamber", HullType.All, 1, 1, 1, RuleBook.BattlefleetKoronus, 40, null, "+2 bonus to the craft rating of all squadrons aboard this starship", max: 1));
            comps.Add(new Supplemental("Plasma Scoop", HullType.Raider | HullType.Frigate, 2, 3, 3, RuleBook.BattlefleetKoronus, 39, null, "May attempt a +0 Pilot+Manoeuvrability Test to gather fuel from a a gas giant. Failure causes 1d5 Hull Integrity damage per degree of failure, ignoring Void Shields. Success allows the vessel to stay at void for an additonal month without needing to refuel, and adds +25 achievement points to any objective that requires the ship to move or transport something", max: 1));
            comps.Add(new Supplemental("Plasma Scoop", HullType.LightCruiser | HullType.LightCruiser | HullType.BattleCruiser, 3, 4, 3, RuleBook.BattlefleetKoronus, 39, null, "May attempt a +0 Pilot+Manoeuvrability Test to gather fuel from a a gas giant. Failure causes 1d5 Hull Integrity damage per degree of failure, ignoring Void Shields. Success allows the vessel to stay at void for an additonal month without needing to refuel, and adds +25 achievement points to any objective that requires the ship to move or transport something", max: 1));
            comps.Add(new Supplemental("Power Ram", HullType.AllCruiser, 2, 0, 2, RuleBook.BattlefleetKoronus, 38, new DiceRoll(1, 0, 0), "External", max: 1));
